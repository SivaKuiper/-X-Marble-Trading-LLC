...
// ═══════════════════════════════════════════════════════════════
// QUOTE COMPARE — extracted as proper component (hooks rule)
// ═══════════════════════════════════════════════════════════════
              ...
              <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:6,marginBottom:10}}>
                    {[["Terms",<Tag key="t" color={C.teal} text={q.terms}/>],["Lead",q.lead+"d"],["MOQ",q.moq+" m2"],["Shade",q.shade],["Rating","* "+q.rating],["On-Time",q.ontime+"%"]].map(([l,v])=>(
...
// ═══════════════════════════════════════════════════════════════
// SALES KPI DASHBOARD — extracted as proper component
// ═══════════════════════════════════════════════════════════════
function SalesKPI() {
    ...
    return (
      <Card style={{padding:"14px 18px"}}>
        ...
        <div style={{position:"relative",height:20,borderRadius:4,overflow:"hidden",background:"#0d0d0f"}}>
          ...
          <div style={{position:"absolute",left:0,top:"25%",height:"50%",width:`${vPct}%`,background:color,borderRadius:3}}/>
        ...
        <div style={{display:"flex",justifyContent:"space-between",marginTop:4,fontSize:9,color:C.textDim}}>
          <span>0</span>
          <span style={{color:vPct>=tPct?C.green:C.amber,fontWeight:600}}>{vPct>=tPct?"up":"down"} {Math.abs(vPct-tPct).toFixed(0)}% vs target</span>
          <span>{max}{unit}</span>
        </div>
      </Card>
    );
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:10}}>
        {[
          ["Revenue MTD","AED 6.7M","vs AED 5.0M target",C.green,"up"],
          ["Revenue vs LY","+"+revVsLY+"%","YoY growth",C.teal,"up"],
          ["Gross Profit","AED 2.68M",gpPct+"% GP margin",C.gold,"$"],
          ["Active Quotes","14","4 expiring this week",C.blue,"~"],
          ["Win Rate","68%","vs 60% target",C.green,"o"],
          ...
          <Card key={l} style={{padding:"12px 14px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
              ...
              <span style={{fontSize:14,color:col}}>{ic}</span>
            </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:14}}>
        <Card>
          ...
          <div style={{display:"flex",gap:16,marginTop:6,justifyContent:"center"}}>
            {[[C.gold,"Revenue"],[C.green,"Gross Profit"],[C.red,"Target"]].map(([c,l])=>(
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          <Card>
            ...
            {[["Emaar Properties","AED 220K",C.gold],["Aldar Properties","AED 185K",C.blue],["Damac Interiors","AED 142K",C.green],["Al Futtaim","AED 98K",C.purple]].map(([n,v,c],i)=>(
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
        ...
        <Card style={{background:"linear-gradient(160deg,#0e1200,#18181b)",border:`1px solid ${C.goldDim}`}}>
          ...
          {[["4 quotes expiring in 7 days",C.amber,"Follow up"],["Statuario Marble stock low",C.red,"Create PO"],["Emaar Tower A accepted",C.green,"Convert order"],["Aldar Q2 tender open",C.blue,"Prepare bid"]].map(([t,c,a])=>(
